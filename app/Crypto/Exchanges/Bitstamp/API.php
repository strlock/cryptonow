...
class API
{
    ...
    private $clientId;
    private $certificate;
    private $publicApiList;
    private $privateApiList;
    private $baseUrl;
    ...
    private function doRequest($type, $api, $params)
    {
        ...
        if($type == "POST")
        {
            $options[CURLOPT_URL]  = $this->baseUrl . $api . "/".(!is_null($currency_pair) ? '/'.$currency_pair : '');
            $options[CURLOPT_POST] = true;
            $options[CURLOPT_POSTFIELDS] = $postFields;
        }
        else
        {
            $options[CURLOPT_URL] = $this->baseUrl . $api . '/'.(!is_null($currency_pair) ? $currency_pair.'/' : '').'?' . $postFields;
        }
     ...
     *    is represented as a list of price and amount.
     */
    public function transactions($time = "hour", $currency_pair = null)
    {
        $params = Array("time"=> $time);
        if (!is_null($currency_pair)) {
            $params['currency_pair'] = $currency_pair;
        }
        return $this->callPublicApi(__FUNCTION__, $params);
    }
